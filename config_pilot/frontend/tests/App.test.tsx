...
const apiPayloads: Record<string, unknown> = {
  "/api/workspaces": [
    { id: "share-workspace", name: "Workspace", root: "/share/config_pilot_workspace", readable: true, writable: true, sensitive: false },
  ],
  "/api/diagnostics": {
  ...
  "/api/codex/status": { installed: false, version: null, path: null, authState: "unknown", codexHome: "/data/config_pilot/.codex" },
  "/api/terminals": [],
...
it("renders the Config Pilot title", async () => {
  render(<App />);
  expect(screen.getByRole("heading", { name: "Config Pilot" })).toBeInTheDocument();
