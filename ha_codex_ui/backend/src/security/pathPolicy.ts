...
export function isSensitivePath(resolvedPath: string): boolean {
  ...
  if (lower === "/data/ha_codex_ui/.codex" || lower.startsWith("/data/ha_codex_ui/.codex/")) {
    return true;
  }
...
export function assertReadAllowed(input: PathPolicyInput): void {
  if (!isReadAllowed(input)) {
    throw new SafeError("READ_BLOCKED", "Reading this path is blocked by the HA_Codex_UI safety policy.", 403);
  }
}

export function assertWriteAllowed(input: PathPolicyInput): void {
  if (!isWriteAllowed(input)) {
    throw new SafeError("WRITE_BLOCKED", "Writing this path is blocked by the HA_Codex_UI safety policy.", 403);
  }
}

export function assertDownloadAllowed(input: PathPolicyInput): void {
  if (!isDownloadAllowed(input)) {
    throw new SafeError("DOWNLOAD_BLOCKED", "Downloading this path is blocked by the HA_Codex_UI safety policy.", 403);
  }
}
...
export function redactPathForLog(inputPath: string): string {
  ...
  if (normalized.startsWith("/data/ha_codex_ui")) {
    return normalized.replace("/data/ha_codex_ui", "/data/ha_codex_ui");
  }
  return normalized;
}
