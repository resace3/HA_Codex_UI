...
describe("WorkspaceService", () => {
  it("lists configured workspaces and marks sensitive roots", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ha-codex-ui-workspaces-"));
  ...
  it("creates only configured writable workspaces", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ha-codex-ui-create-"));
