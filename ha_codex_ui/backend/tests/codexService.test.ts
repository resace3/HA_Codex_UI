...
describe("CodexService", () => {
  it("detects missing auth without exposing files", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ha-codex-ui-codex-"));
  ...
  it("detects likely auth from file names only", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "ha-codex-ui-codex-auth-"));
